...
- ⚡ 玩家网页商店提供「一键售卖」,按配置卖价回收背包中全部可售普通商品并一次结算
...
| 命令 | 别名 | 说明 |
| --- | --- | --- |
| `/buy <id> [数量]` | | 购买:普通商品填**物品 ID**,礼包填**礼包 id**;省略数量时默认为 1 |
| `/sell <id> [数量]` | | 出售:普通商品填**物品 ID**,礼包填**礼包 id**;省略数量时默认为 1 |
...
**普通商品与礼包分块管理**:普通商品(物品 ID + 买卖价,瓦片直接显示价格)、礼包(id + 名称 + 内容 + 买卖价)、
搜索游戏物品并**一键加入商店(点击弹出买/卖价输入框)**、配置折扣分级。未安装面板时插件照常通过命令工作。

此外还会注册一个**面向玩家**的商店菜单:玩家在游戏内输入 `/menu`(或 `/menu shop`)即可在浏览器里(分区
列表)浏览全部商品、点按钮购买或出售,沿用与 `/buy`、`/sell` 完全一致的折扣、扣费与失败退款逻辑。

页面顶部的「一键售卖」会立即回收背包中所有 `sellPrice > 0` 的普通商品并汇总结算；为避免礼包内容
与普通商品重叠而产生歧义,礼包不会被该按钮自动出售,仍需逐项手动出售。
