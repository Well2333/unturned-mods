...
`warpMap.visibility: native` 是推荐值；它分别检查原生 Chart 与 GPS/卫星图资格。设为 `always` 会让玩家网页不再要求这两类资格。无论哪种模式，只有已登录、在线且有传送点权限的玩家能看到对应标记。`Chart.png` 与 `Map.png` 各自限制为 64 MiB，并独立使用私有缓存与 ETag，替换任一图片后会自动更新。
