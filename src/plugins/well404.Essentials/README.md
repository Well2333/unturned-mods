...
warps:
  - name: "spawn"
    category: "public"     # 玩家面板只读标签，可用于过滤
    order: 1               # 管理员面板在同标签内拖动排序
    x: 0
    y: 20
    z: 0
    yaw: 0
...
管理员可在 WebPanel 的传送点标签页设置 `category`，并在同一标签内拖动排序。玩家面板只显示这些标签并用于过滤，不能编辑。旧传送点自动进入 `default` 并保留原有列表顺序；`/warp set` 更新坐标时不会清除既有标签和顺序。
