...
function syncMapRefreshPause() {
  const spatialView = warpState.view === "gps" || warpState.view === "chart";
  panel.setAutoRefreshPaused?.(active === "warps" && spatialView);
}
select(active);
...
function installPanZoom(stage, canvas, markers, controls) {
  ...
  const reset = () => { scale = 1; x = 0; y = 0; render(); persist(); };
  const wheel = event => {
...
function renderMap(kind, visible) {
  ...
  image.onerror = () => {
    clearMap();
    mapHost.replaceChildren(make("div", "map-empty", zh ? "地图图片加载失败，请切换视图。" : "The map image failed to load. Switch views."));
  };
  let count = 0;
  ...
  disposeMap = installPanZoom(stage, canvas, markers, { zoomIn, zoomOut, reset });
}
...
function warpField(form, label, name, value, type = "number", disabled = false) {
  ...
  if (type === "text") {
    const limits = { name:64, map:128, id:32, nameEn:64, nameZh:64, emoji:16 };
...
panel.onDispose?.(() => {
  clearMap();
  panel.setAutoRefreshPaused?.(false);
});
