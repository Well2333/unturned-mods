    ...
    public class VaultService
    {
        ...
        public VaultContainerSnapshot GetContainer(VaultContainerRef container)
        {
            ...
            var baseSlots = container.Kind == VaultOwnerKind.Team
                ? Math.Max(1, TeamSettings.BaseSlots)
                : BaseMaxSlots;
        ...
        public VaultContainerSnapshot? SetContainerCapacity(VaultContainerRef container, int capacity)
        {
            ...
            return Store.SetContainerCapacity(
                container, existing?.DisplayName ?? string.Empty, baseSlots, Math.Max(1, capacity));
        }
            ...
            UnturnedUser user, ushort itemId, int amount, byte? itemAmount = null, byte? quality = null, string? state = null)
        {
            ...
            return Store.MoveMany(
                ...
                Math.Max(1, teamSettings.BaseSlots), Math.Max(team.Value.Container.Capacity, Math.Max(teamSettings.BaseSlots, teamSettings.MaxSlots)),
            ...
            UnturnedUser user, ushort itemId, int amount, byte? itemAmount = null, byte? quality = null, string? state = null)
        {
            ...
            return Store.MoveMany(
                ...
                Math.Max(1, TeamSettings.BaseSlots),
                VaultContainerRef.Player(user.Id), user.DisplayName, personalBase,
                Math.Max(personal.Capacity, Math.Max(personalBase, personalPurchase.MaxSlots)),
                item => MatchesVariant(item, itemId, itemAmount, quality, state), Math.Max(0, amount));
        }
            ...
            int? expectedMaximum)
        {
            ...
            var baseSlots = await GetPersonalBaseSlotsAsync(user);
            var maximum = Math.Max(baseSlots, purchase.MaxSlots);
            if (!purchase.Enabled)
                return new TeamVaultPurchaseResult(TeamVaultPurchaseStatus.Disabled, 0, 0, purchase.Price);
            if (purchase.MaxSlots < 1 || purchase.SlotsPerPurchase < 1 || purchase.Price < 0m)
            ...
            int? expectedMaximum)
        {
            ...
            if (!purchase.Enabled)
                return new TeamVaultPurchaseResult(TeamVaultPurchaseStatus.Disabled, 0, 0, purchase.Price);
            if (settings.BaseSlots < 1 || settings.MaxSlots < settings.BaseSlots
                || purchase.SlotsPerPurchase < 1 || purchase.Price < 0m)
            {
                return new TeamVaultPurchaseResult(
                    TeamVaultPurchaseStatus.InvalidConfiguration, 0, 0, purchase.Price);
            }
        ...
        public async Task<int> RecoverPendingTeamPurchasesAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                return await RecoverPendingTeamPurchasesCoreAsync(cancellationToken);
            }
            finally
            {
                m_PurchaseRecoveryGate.Release();
        ...
        private async Task<int> RecoverPendingTeamPurchasesCoreAsync(CancellationToken cancellationToken = default)
        {
            ...
            var recovered = 0;
            foreach (var pending in Store.GetPendingTeamPurchases())
            {
                ...
                try
                {
                    ...
                    if (string.Equals(state, "debiting", StringComparison.Ordinal))
                    {
                        if (pending.Price > 0m)
                        {
                            ...
                            var refundExists = (await durableEconomy.GetAppliedBalanceAsync(
                                pending.OperationId + ":refund", pending.BuyerSteamId,
                                actorType, pending.Price)).HasValue;
                            if (refundExists)
                            {
                            ...
                            var debitExists = (await durableEconomy.GetAppliedBalanceAsync(
                                pending.OperationId, pending.BuyerSteamId,
                                actorType, -pending.Price)).HasValue;
                    ...
                    if (string.Equals(state, "debited", StringComparison.Ordinal))
                    {
                        if (pending.Container.Kind == VaultOwnerKind.Team)
                        {
                            ...
                            var lookup = await teamProvider.GetCurrentTeamAsync(
                                pending.BuyerSteamId, actorType);
                    ...
                    if (string.Equals(state, "ready", StringComparison.Ordinal))
                    {
                        ...
                        var maximum = pending.Container.Kind == VaultOwnerKind.Team
                            ? Math.Max(TeamSettings.BaseSlots, TeamSettings.MaxSlots)
                            : Math.Max(snapshot?.BaseCapacity ?? BaseMaxSlots,
            ...
            string reason)
        {
            if (price > 0m)
            {
                ...
                var refundExists = (await economy.GetAppliedBalanceAsync(
                    operationId + ":refund", buyerId, buyerType, price)).HasValue;
                if (!refundExists)
                {
                    var debitExists = (await economy.GetAppliedBalanceAsync(
                        operationId, buyerId, buyerType, -price)).HasValue;
                    if (debitExists)
                    {
                        await economy.ApplyOnceAsync(
        ...
        {
            var purchase = CurrentPersonalPurchaseSettings;
            var maximum = Math.Max(Math.Max(1, baseSlots), purchase.MaxSlots);
