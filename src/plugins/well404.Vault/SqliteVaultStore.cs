...
SELECT id, owner_kind, owner_key, display_name, base_capacity, purchased_slots,
       admin_adjustment, used_slots, version, purchase_version,
...
WHERE id = $id;";
                command.Parameters.AddWithValue("$capacity", Math.Max(1, capacity));
...
SELECT container.id, container.owner_kind, container.owner_key, container.display_name,
       container.base_capacity, container.purchased_slots, container.admin_adjustment, container.used_slots,
    ...
    updated_utc = excluded.updated_utc;";
            ...
            IReadOnlyList<StoredItem> items)
        {
            ...
            WithDatabase(connection =>
            {
                ...
                IncrementUsedSlots(connection, transaction, containerId, SumSlots(items));
                transaction.Commit();
                return 0;
            });
        }
...
SET used_slots = used_slots + $cost,
    ...
    updated_utc = $updated
WHERE id = $id
  AND used_slots + $cost <= MIN(base_capacity + purchased_slots + admin_adjustment, $maximum);";
                    ...
                    reserve.Parameters.AddWithValue("$maximum", Math.Max(1, maximumCapacity));
...
SET purchased_slots = purchased_slots + $slots,
    version = version + 1,
    updated_utc = $updated
WHERE id = $id AND base_capacity + purchased_slots + admin_adjustment + $slots <= $maximum;";
...
ORDER BY purchase.created_utc;";
                ...
                while (reader.Read())
                {
                    result.Add(new PendingTeamVaultPurchase(
                        reader.GetString(0),
                        reader.GetString(1),
...
ON CONFLICT(owner_kind, owner_key) DO UPDATE SET
    ...
    updated_utc = excluded.updated_utc;";
                ...
                upsert.Parameters.AddWithValue("$baseCapacity", Math.Max(1, baseCapacity));
