...
- 🧭 支持按 Unturned 原生物品类型与蓝图语义筛选弹药、食物与饮品、药品、武器、材料、工具、服装、配件、建筑、
  载具用品和其他；弹匣以及带 Ammo/FillTargetItem 蓝图的弹药箱都归入弹药，
  原版与创意工坊共用同一规则，未知新类型安全归入「其他」
- 💾 背包/仓库标签、排序方式、排序方向和筛选条件在当前浏览器会话内保留
...
管理员 WebPanel 的「个人仓库 → 玩家仓库」可输入 17 位 SteamID，加载该玩家在 SQLite 中的精确记录。每条记录可修改物品 ID、数量和品质；修改时会按新物品重新计算格子占用与弹药容量，同时保留不透明的原始 `state` 数据。单条删除和“按物品 ID 删除该玩家的全部匹配项”都会二次确认，服务端还会同时校验 SteamID 与记录 ID，不能越界修改其他玩家。
