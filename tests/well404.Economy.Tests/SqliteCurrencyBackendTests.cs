...
namespace well404.Economy.Tests
{
    public class SqliteCurrencyBackendTests : IDisposable
    {
        ...
        [Fact]
        public async Task Transfer_MovesBothBalancesInOneTransaction()
        {
            var backend = NewBackend();
            await backend.SetBalanceAsync("sender", Player, 100m);
            await backend.SetBalanceAsync("receiver", Player, 10m);

            await backend.TransferAsync(
                "sender", Player, "receiver", Player, 25m, 20m, "pay:test");

            Assert.Equal(75m, await backend.GetBalanceAsync("sender", Player));
            Assert.Equal(30m, await backend.GetBalanceAsync("receiver", Player));
            Assert.Equal(4L, ScalarLong("SELECT COUNT(*) FROM transactions;"));
        }

        [Fact]
        public async Task Transfer_InsufficientBalance_RollsBackBothAccounts()
        {
            var backend = NewBackend();
            await backend.SetBalanceAsync("sender", Player, 5m);
            await backend.SetBalanceAsync("receiver", Player, 10m);

            await Assert.ThrowsAsync<NotEnoughBalanceException>(() => backend.TransferAsync(
                "sender", Player, "receiver", Player, 25m, 20m, "pay:test"));

            Assert.Equal(5m, await backend.GetBalanceAsync("sender", Player));
            Assert.Equal(10m, await backend.GetBalanceAsync("receiver", Player));
            Assert.Equal(2L, ScalarLong("SELECT COUNT(*) FROM transactions;"));
        }

        [Fact]
        public async Task SetBalance_RejectsNegativeValue()
        {
            var backend = NewBackend();
            await Assert.ThrowsAsync<ArgumentOutOfRangeException>(
                () => backend.SetBalanceAsync("a", Player, -1m));
        }
