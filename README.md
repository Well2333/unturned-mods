...
| 插件 | 说明 |
| --- | --- |
| `well404.Economy` | 经济系统：以全局 `IEconomyProvider` 暴露货币；后端可选 SQLite 事务账本或原生经验值；支持 `/pay` 转账、`/balance`、`/eco` 管理、以及击杀（玩家/僵尸/动物）奖励。其他插件（签到等）可注入 `IEconomyProvider` 发币。 |
| `well404.Shop` | 分组物品商店：`config.yaml` 配置商品价格、分组、备注和顺序；`/buy` `/sell` `/shop`；按权限组的购买折扣（如 VIP 9 折，默认关闭）。**硬依赖 `well404.Economy`**,`openmod install` 会自动一并安装。 |
| `well404.WebPanel` | 通用 Web 管理面板：内置零依赖 HTTP 服务 + 单页应用,经 `IWebPanelRegistry` 让各插件挂载可视化管理模块(设置组、集合 CRUD、搜索)。可选,装上后 Economy / Shop / Essentials 自动出现管理界面。还有可选内置反代(cloudflared/ngrok,需管理员预装 cloudflared)。 |
...
| `well404.Vault` | 个人与小队共享仓库：完整保真存取背包物品，容量按格子计；Essentials 提供实时小队身份，SQLite Economy 可让成员购买最高 5000 格共享容量，WebPanel 提供玩家与管理员界面。 |
...
# 构建全部 -> 默认平铺输出到 build/（插件 dll + 非宿主第三方依赖，如 Microsoft.Data.Sqlite.dll）
scripts/build.sh
