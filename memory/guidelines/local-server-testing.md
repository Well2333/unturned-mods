...
> **最高优先级环境规则：** 不需要用户本人参与的服务器验证只能使用本地 `.localserver`，不得启动
> 远程 `TestServer`。远程 `TestServer` 仅用于用户需要进服/查看/操作的实机步骤。无论本地还是远程
> 测试服，只要没有正在等待用户参与，验证结束后必须立即关闭，禁止常驻。
...
3. **其他对外接口**：不依赖游戏客户端即可验证的面（如 `well404.WebPanel` 的 HTTP API，
   ...
   鉴权、返回结构、增删改是否落库/落配置）。
4. **干净卸载/关服**：`stop` 后无报错、无残留锁文件（如 SQLite WAL 文件未正常收敛）。

第 4 项不是可选收尾：纯自测完成后必须执行 `stop` 并确认进程退出。只有用户已明确要立即参与
后续实机检查时，远程 `TestServer` 才可在该人工检查窗口内暂时保持运行。
