...
- **检索游戏物品**：按名称或 ID 查物品，便于填礼包内容。

> 在 WebUI 新建传送点后，记得给玩家授予对应的 `well404.essentials.warps.<名称>` 权限。

## 玩家网页便民菜单（可选）

装了 `well404.WebPanel` 后，Essentials 还会注册一个**面向玩家**的便民菜单（玩家面 `/p`，
图标 🧭）。玩家在游戏内输入 `/menu`（或 `/menu essentials`）收到链接，在浏览器里一键：

- **回家**（未设置家时提示用 `/sethome`）、**返回死亡点**（有记录时才出现）；
- 传送到任一**自己有权限**的传送点（逐个按 `well404.essentials.warps.<名称>` 过滤）。

所有传送都走与命令相同的 `TeleportService` 流程（冷却、可选经济费用、静止预热），因此点按钮
后可能需要在游戏里短暂站立读条；具体失败原因（冷却中、余额不足、移动取消）仍在游戏内提示。
