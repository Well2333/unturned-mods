...
管理端用顶部标签在传送设置、其他规则、传送点、礼包和物品检索之间切换；玩家「实用工具」也只展示当前选中的功能标签，5 秒刷新后保留选中项。
...
管理员可在「Essentials → 传送点」为每个传送点设置 `category` 标签，并在同一标签页内拖动卡片排序。玩家只能查看标签并按“全部/标签”过滤，不能编辑或拖动。旧配置中没有 `category` 和 `order` 的传送点会自动按原列表顺序归入 `default`，无需手工迁移。`/warp set` 更新已有传送点坐标时会保留标签和顺序。
