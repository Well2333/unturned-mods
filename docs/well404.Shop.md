...
- 顶部「快捷操作」提供红色的**一键售卖**按钮:点击后立即扫描背包,把所有在 `items` 中配置了
  `sellPrice > 0` 的普通商品全部回收,按各自卖价汇总后一次性入账;
- 点「购买」/「出售」并输入数量即可成交——沿用与 `/buy`、`/sell` 完全一致的折扣、扣费、
  失败退款与背包校验逻辑(玩家须在线)。

> 「一键售卖」不自动出售礼包。礼包内容可能与普通商品或其他礼包重叠,若自动处理会让结算结果取决于
> 配置顺序;因此礼包继续使用礼包行上的「出售」按钮手动处理。该按钮为无确认的一键操作,点击前请确认
> 背包中的可售普通商品均要出售。
