...
- 顶部以一枚徽章显示自己的当前余额;
- 商品以**自适应的商品网格**(product tiles)排布(窄屏单列、宽屏多列),买入按钮为醒目的绿色;
- 每件商品一张卡片,标注买价(若有折扣会显示折后价)与卖价;**单物品**卡片标题即物品名,不再
  显示原始数字 ID;**组合包**单列一行「礼包 —— 包含：」提示,下面用**物品名称 ×数量**的标签逐项列出
  (区别于普通物品,不再把「礼包」与内容混在同一行同样式);
