...
- **搜索**:按名称或 ID 搜索 Unturned 游戏物品(最多 100 条),便于快速填入 `itemId`;输入**纯数字**时
  优先把**该精确 ID** 的物品列在最前(即使有大量 ID 含这些数字也能找到)。
...
- 顶部以一枚徽章显示自己的当前余额,右侧有「**卡片 / 列表**」视图切换(记忆在浏览器本地);
- **单品**与**组合包(礼包)分区展示**(各带数量计数),两种视图都尽量紧凑、减少留白,适配 Steam
  内嵌浏览器的窄屏;
- **卡片视图**:电商式商品瓦片网格(窄屏自动减列),标题为物品名、买价大号绿色突出(有折扣显示
  折后价 + 划线原价),按钮「购买 / 出售 $价」;礼包瓦片用**胶囊标签**逐项列出内容(物品名 ×数量);
- **列表视图**:紧凑行,响应式单/双列。单品行 = `#物品ID | 物品名 | 购买 $买价 | 出售 $卖价`(价格即按钮,
  点击买/卖);礼包行(整行更宽)= `礼包名 | 内容胶囊标签 | 购买 | 出售`;不可买/卖的一侧显示 `—`;
